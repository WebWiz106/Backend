...
from usecases import inventory,rooms
...
def hello():
    return jsonify({"Message":"Hi Prices"})
        ...
        for i in room:
            ...
            for date in next40dates:
                if i["isWeekendFormat"]:
                    ...
                    if(inventory.is_weekend(str(date))):
...
def update_room_prices():
    try:
        booking_details = request.get_json(force=True)
        status = inventory.update_price_of_rooms(booking_details)
...
def bulk_Update_prices():
    try:
        booking_details = request.get_json(force=True)
        status = inventory.update_bulk_price_of_rooms(booking_details)
        return jsonify({"Status":status})
    except Exception as ex:
        return ({"Message": "{}".format(ex)}), 500
