...
def create_booking(booking_details):
    try:
        ...
        db.Webjini_Bookings.insert_one(Bookings.to_dict(booking))
...
def get_booking_id():
    try:
        ...
        print("entered")
        booking_count = str(db.Webjini_Bookings.count_documents(query)+1)
        print(booking_count)
...
def get_all_bookings(token, hId, type):
    try:
        ...
        if type == "0":
            bookings = db.Webjini_Bookings.find({"jiniId": jiniid, "hId": hId})
        else:
            bookings = db.Webjini_Bookings.find({"jiniId": jiniid, "hId": hId, "Bookings": {
                "$elemMatch": {
                    "RoomType": type,
                    "Qty": {"$gt": 0}
                }
...
def calculate_booking_total(booking_details, ndid, hId):
    try:
        ...
        room = rooms.get_room(ndid, hId, room_type)
        if room:
